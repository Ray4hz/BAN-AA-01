...
data raw; 
	set r_dm; 
	drop Obs enrollment investigator; 
run; 
...
12. RACE	= race
13. ETHNIC	= ethnic
14. ARMCD 	= A (treatment = 1) or P (treatment = 2)
15. ARM 	= Drug A (treatment = 1) or Placebo (treatment = 2)
16. COUNTRY = location
***************************************************************/ 
...
data s_prog.dm; 
	...
	keep 	STUDYID DOMAIN USUBJID SUBJID RFSTDTC RFENDTC 
			SITEID BIRTHDTC AGE AGEU SEX RACE ETHNIC ARMCD ARM COUNTRY; 
	...
	attrib BIRTHDTC length = $64	label = "Date/time of birth of the subject"; 
...
* derive SDTM DM variables; 
	...
	d = DAY(input(birth, yymmdd8.)); 
	m = MONTH(input(birth, yymmdd8.)); 
	y = YEAR(input(birth, yymmdd8.)); 
	BIRTHDTC = put( MDY(m, d, y), is8601da.); 
	AGE 	= age_year; 
	AGEU	= "YEARS"; 
	SEX 	= "M"; 
...
%toexcel(indsn=s_prog.dm, name=dm, path=C:\bancova\projects\prostate\data\sdtm\prog); 
