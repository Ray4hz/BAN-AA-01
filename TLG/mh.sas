...
| PROGRAMMER(S):    Ray
| DATE:             
| PURPOSE:          Generate medical history table
|                        
...
proc import 	datafile = "&dDir\medical\mh.xls"
				out 	 = rawmh
				dbms 	 = xls replace; 
	sheet 		= "mh";
	getnames 	= yes;  
...
proc sql; 
	create table mh1 as
	select strip(a.USUBJID) as usubjid, strip(a.MHTERM) as ptterm, strip(a.MHSCAT) as soct, b.treatment, b.SAFETY
		from rawmh as a
			left join
				rawdm as b
...
proc sql noprint; 
	select count(distinct usubjid) into: none1 - : none3 from mh group by treatment; 
quit; 
...
** use macro to count; 
%catstat2(indsn=mh, outdsn=cntptt, grp=treatment, var=soc ptt); 

%pt(cntptt); 

%catstat2(indsn=mh, outdsn=cntsoc, grp=treatment, var=soc); 
...
data final; 
	...
	set outsort; 
	if missing(soc) then soct = "Patients with Abnormalities in any System Organ Class"; 
...
%let task = MH; 
%let tableno = Table 3; 
%let title = Medical and Surgical History/Physical Findings; 
...
proc report data = final nowindows missing headline headskip split="|" 
	...
	** Display the values; 
	...
	compute after footnote;
     	line "^S={font_size=8pt just=l leftmargin=0.1in font_face=arial} Notes:   N = Number of patients in the Safety Population."; 
		line "^S={font_size=8pt just=l leftmargin=0.1in font_face=arial} ^\tab n = Number of patients with abnormality."; 
	endcomp;
run; 
